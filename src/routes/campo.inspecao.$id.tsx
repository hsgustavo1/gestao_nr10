...
function CampoInspecaoPage() {
  ...
  function descendentes(nodeId: string): Set<string> {
    ...
    while (fila.length) {
      ...
      for (const c of childrenByParent.get(cur) ?? []) {
        if (!out.has(c.id)) { out.add(c.id); fila.push(c.id); }
      }
    }
    return out;
  }
  ...
  function contagem(nodeId: string): { pontos: number; achados: number } {
    ...
    let pontos = 0, achados = 0;
    for (const p of points) {
      if (ids.has(p.node_id)) { pontos += 1; achados += p._achados; }
    }
  ...
  if (isLoading || !inspection) {
    return (
      <PageShell>
        ...
        <div className="mt-4 space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-16" />)}</div>
      </PageShell>
    );
  }
  ...
  return (
    <PageShell>
      ...
      <div className="mt-2 flex items-start justify-between flex-wrap gap-2">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-xs text-muted-foreground flex flex-wrap gap-x-3 gap-y-0.5">
            ...
            <span className="font-semibold text-primary">{totalAchados} achado{totalAchados !== 1 ? "s" : ""}</span>
          </div>
        </div>
      ...
      <div className="mt-3 flex items-center gap-1 text-sm overflow-x-auto whitespace-nowrap pb-1">
        <button type="button" onClick={() => setCurrentNodeId(null)} className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 ${currentNodeId === null ? "font-semibold text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        {path.map((n, i) => {
          ...
          return (
            <span key={n.id} className="inline-flex items-center gap-1">
              ...
              <button
                type="button" onClick={() => setCurrentNodeId(n.id)}
      ...
      {isStaff && !jaImportada && (
        <div className="mt-2">
          {podeColetar ? (
            <Button onClick={() => setCapturaOpen(true)} className="w-full h-12 text-base bg-brand-gradient text-white shadow-brand">
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            ...
            {isStaff && !jaImportada && (
              <div className="flex gap-1">
                {currentNodeId === null && (
                  <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setCargaOpen(true)}>
                    <Upload className="h-3.5 w-3.5" /> Carregar setores
                  </Button>
                )}
                <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => setAddNodeOpen(true)}>
          ...
          {children.length === 0 ? (
            <div className="rounded-md border border-dashed bg-muted/10 p-4 text-center text-xs text-muted-foreground">
              ...
              {currentNodeId === null && " Crie manualmente, carregue por planilha ou use a base histórica."}
            </div>
          ) : (
                ...
                return (
                  <Card key={n.id} className="transition-colors hover:border-primary/40">
                    <CardContent className="p-0">
                      <button type="button" onClick={() => setCurrentNodeId(n.id)} className="flex items-center gap-3 p-3 w-full text-left">
            ...
            <div className="space-y-2">
              {pontosAqui.map((p) => (
                <Card key={p.id} className="transition-colors hover:border-primary/40">
                  <CardContent className="p-0">
                    <Link to="/campo/ponto/$id" params={{ id: p.id }} className="flex items-center gap-3 p-3">
                      <div className="min-w-0 flex-1">
                        <div className="font-medium leading-tight truncate">{p.titulo || `Ponto ${p.ordem || ""}`.trim() || "Ponto de coleta"}</div>
                        <div className="text-[11px] text-muted-foreground">
                          {p._achados} modo{p._achados !== 1 ? "s" : ""} de falha · {p._fotos} foto{p._fotos !== 1 ? "s" : ""}
                        </div>
                      </div>
                      {p._fotos > 0 && <span className="inline-flex items-center gap-1 text-[11px] text-primary shrink-0"><Camera className="h-3.5 w-3.5" />{p._fotos}</span>}
                      <ChevronRight className="h-4 w-4 text-muted-foreground shrink-0" />
                    </Link>
                  </CardContent>
                </Card>
      ...
      {isStaff && capturaOpen && currentNodeId && (
        <CapturaPontoSheet
          ...
          onOpenChange={(o) => { if (!o) setCapturaOpen(false); }}
        />
      )}
      {isStaff && addNodeOpen && childLevel && (
        <AddNodeDialog
          ...
          onOpenChange={(o) => { if (!o) setAddNodeOpen(false); }}
        />
      )}
      {isStaff && cargaOpen && (
...
function DeleteNodeButton({ node }: { node: FieldNode }) {
  ...
  return (
    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-muted-foreground shrink-0" onClick={handle} title="Excluir">
...
function CapturaPontoSheet({
  ...
  function addFotos(list: FileList | null) {
    ...
    const novos = [...list].filter((f) => f.type.startsWith("image/") || f.type === "application/pdf");
  ...
  async function salvar(e: FormEvent) {
    e.preventDefault();
    if (totalAchados === 0) return toast.error("Selecione ao menos um modo de falha ou adicione um achado manual.");
    setBusy(true);
    ...
    try {
      ...
      const achados: AchadoNovo[] = [
        ...[...selecionados].map((id) => {
          ...
          return {
            modo_falha_id: m.id, descricao: m.descricao_padrao, recomendacao: m.recomendacao_padrao,
            prioridade: m.prioridade_sugerida, tipo_execucao: m.tipo_execucao_sugerido, observacao: null,
          } as AchadoNovo;
        }),
        ...manuais.map((m) => ({ ...m, modo_falha_id: null })),
      ];
      await criar.mutateAsync({
        inspectionId, nodeId, titulo: titulo.trim() || null, observacoes: observacoes.trim() || null,
        ordem: proximaOrdem, fotos: enviadas, achados,
      });
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!busy) onOpenChange(o); }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="outline" className="h-11" onClick={() => galeriaRef.current?.click()}>
            ...
            {previews.length > 0 ? (
              <div className="mt-2 grid grid-cols-4 gap-2">
                {previews.map((src, i) => (
                  <div key={i} className="relative aspect-square rounded-md overflow-hidden border">
                    ...
                    <button type="button" onClick={() => removeFoto(i)} className="absolute top-0.5 right-0.5 grid h-5 w-5 place-items-center rounded-full bg-black/60 text-white">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
              ...
              <p className="mt-1.5 text-[11px] text-muted-foreground">A foto é opcional (ex.: NC de documentação não tem o que fotografar).</p>
            )}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            <div className="space-y-1">
              <Label htmlFor="cp-titulo" className="text-xs">Título do ponto (opcional)</Label>
              <Input id="cp-titulo" value={titulo} onChange={(e) => setTitulo(e.target.value)} maxLength={150} placeholder="Ex.: vista frontal do painel" className="h-9" />
            </div>
            <div className="space-y-1">
              <Label htmlFor="cp-obs" className="text-xs">Observação (opcional)</Label>
              <Input id="cp-obs" value={observacoes} onChange={(e) => setObservacoes(e.target.value)} maxLength={300} className="h-9" />
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <Label className="text-xs font-semibold inline-flex items-center gap-1"><ListChecks className="h-3.5 w-3.5" /> Modos de falha ({totalAchados})</Label>
              <Button type="button" size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setManualOpen(true)}>
            ...
            {manuais.length > 0 && (
              <div className="mb-2 space-y-1">
                {manuais.map((m, i) => (
                  <div key={i} className="flex items-start gap-2 rounded-md border border-slate-300 bg-slate-50 p-2 text-xs">
                    <span className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[10px] font-bold ${RTI_PRIORIDADE_BADGE[clampPrioridade(m.prioridade)]}`}>P{m.prioridade}</span>
                    <span className="flex-1 leading-snug">{m.descricao}</span>
                    <button type="button" onClick={() => setManuais((prev) => prev.filter((_, idx) => idx !== i))} className="text-destructive"><X className="h-3.5 w-3.5" /></button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="max-h-64 overflow-y-auto rounded-md border divide-y">
              {filtrados.size === 0 ? (
                <p className="p-4 text-center text-xs text-muted-foreground">Nenhum modo encontrado.</p>
              ) : (
                [...filtrados.entries()].map(([categoria, items]) => (
                  <div key={categoria}>
                    <div className="px-2.5 py-1 bg-muted/40 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{categoria}</div>
                    {items.map((m) => {
                      const on = selecionados.has(m.id);
                      return (
                        <label key={m.id} className={`flex items-start gap-2 p-2.5 cursor-pointer ${on ? "bg-primary/[0.06]" : "hover:bg-muted/30"}`}>
                          <Checkbox checked={on} onCheckedChange={() => toggleModo(m.id)} className="mt-0.5" />
                          <div className="min-w-0 flex-1">
                            <div className="text-sm font-medium leading-snug">{m.label}</div>
                            {m.normas.length > 0 && <div className="text-[10px] text-muted-foreground">{formatNormas(m.normas)}</div>}
                          </div>
          ...
          {progresso && (
            <div className="space-y-1">
              <Progress value={progresso.total > 0 ? Math.round((progresso.done / progresso.total) * 100) : 0} className="h-1.5" />
              <p className="text-[11px] text-muted-foreground text-center">Enviando fotos {progresso.done}/{progresso.total}…</p>
            </div>
          )}
        </div>

        <DialogFooter className="p-3 border-t flex-row gap-2">
          <Button type="button" variant="outline" className="flex-1" onClick={() => onOpenChange(false)} disabled={busy}>Cancelar</Button>
          <Button type="button" className="flex-1 bg-brand-gradient text-white shadow-brand" onClick={salvar} disabled={busy || totalAchados === 0}>
      ...
      {manualOpen && (
        <ManualAchadoDialog onAdd={(a) => setManuais((prev) => [...prev, a])} onOpenChange={(o) => { if (!o) setManualOpen(false); }} />
      )}
    </Dialog>
  );
}

function ManualAchadoDialog({ onAdd, onOpenChange }: { onAdd: (a: AchadoNovoUI) => void; onOpenChange: (o: boolean) => void }) {
  ...
  function add(e: FormEvent) {
    ...
    onAdd({ descricao: descricao.trim(), recomendacao: recomendacao.trim() || null, prioridade: Number(prioridade), tipo_execucao: tipo, observacao: null });
    onOpenChange(false);
  }
      ...
      <DialogContent className="w-[calc(100vw-1rem)] sm:max-w-md">
        ...
        <form onSubmit={add} className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Textarea id="ma-desc" value={descricao} onChange={(e) => setDescricao(e.target.value)} rows={3} maxLength={2000} required />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="ma-rec">Recomendação (opcional)</Label>
            <Textarea id="ma-rec" value={recomendacao} onChange={(e) => setRecomendacao(e.target.value)} rows={2} maxLength={3000} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={prioridade} onValueChange={setPrioridade}>
                ...
                <SelectContent>{RTI_PRIORIDADES.map((p) => <SelectItem key={p} value={String(p)}>{RTI_PRIORIDADE_LABELS[p]}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Tipo</Label>
          ...
          <DialogFooter className="flex-col-reverse sm:flex-row gap-2">
            ...
            <Button type="submit" className="bg-brand-gradient text-white shadow-brand">Adicionar</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
function AddNodeDialog({
  inspectionId, parentId, nivel, proximaOrdem, onOpenChange,
}: {
  inspectionId: string;
  ...
  async function submit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      await upsert.mutateAsync({ inspection_id: inspectionId, parent_id: parentId, nivel, nome: nome.trim(), ordem: proximaOrdem });
      ...
      <DialogContent className="w-[calc(100vw-1rem)] sm:max-w-md">
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Input
              id="an-nome" value={nome} onChange={(e) => setNome(e.target.value)} maxLength={150} required autoFocus
              list={nivel === "setor" ? "setores-hist" : undefined}
              placeholder={nivel === "setor" ? "Ex.: CALDEIRA" : nivel === "ativo" ? "Ex.: CCM-02" : "Ex.: Gaveta G4"}
            />
            {nivel === "setor" && (
              <datalist id="setores-hist">{setoresHist.map((s) => <option key={s} value={s} />)}</datalist>
            )}
            {nivel === "setor" && setoresHist.length > 0 && (
              <p className="text-[10px] text-muted-foreground">Comece a digitar para ver sugestões da base histórica.</p>
            )}
          </div>
          <DialogFooter className="flex-col-reverse sm:flex-row gap-2">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Cancelar</Button>
            <Button type="submit" className="bg-brand-gradient text-white shadow-brand" disabled={busy}>{busy ? "Criando..." : "Criar"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
function CargaEstruturaDialog({ inspectionId, onOpenChange }: { inspectionId: string; onOpenChange: (o: boolean) => void }) {
  ...
  async function importarPlanilha(file: File) {
    ...
    try {
      ...
      if (linhas.length === 0) { toast.error("A planilha não tem linhas válidas (coluna Setor)."); return; }
    ...
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-1rem)] sm:max-w-lg flex flex-col p-0">
        <DialogHeader className="p-4 pb-2">
          <DialogTitle className="flex items-center gap-2 leading-tight"><Upload className="h-5 w-5 shrink-0 text-primary" /> Carregar setores</DialogTitle>
          <DialogDescription>Por planilha (Setor/Ativo/Componente) ou da base histórica dos setores reais.</DialogDescription>
        </DialogHeader>

        <div className="flex-1 overflow-y-auto px-4 pb-2 space-y-4">
          {/* Planilha */}
          <div className="rounded-md border p-3 space-y-2">
            <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Por planilha</div>
            <input ref={fileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) importarPlanilha(f); }} />
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => baixarPlanilhaModeloEstrutura(setoresHist)} disabled={busy}>
                Baixar planilha modelo
              </Button>
              <Button type="button" size="sm" className="bg-brand-gradient text-white shadow-brand" onClick={() => fileRef.current?.click()} disabled={busy}>
                <Upload className="h-4 w-4" /> Enviar planilha preenchida
              </Button>
            </div>
            <p className="text-[10px] text-muted-foreground">Colunas: Setor (obrigatório), Ativo, Componente. Cada linha é um caminho; o modelo já vem com os {setoresHist.length} setores reais de referência.</p>
          </div>

          {/* Base histórica */}
          <div className="rounded-md border p-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Da base histórica ({selecionados.size})</div>
              <button type="button" className="text-[11px] text-primary" onClick={() => setSelecionados(new Set(filtrados))}>Marcar todos</button>
            </div>
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
              <Input placeholder="Buscar setor..." className="pl-8 h-8 text-sm" value={busca} onChange={(e) => setBusca(e.target.value)} />
            </div>
            <div className="max-h-52 overflow-y-auto space-y-0.5">
              {filtrados.map((s) => {
                const on = selecionados.has(s);
                return (
                  <label key={s} className={`flex items-center gap-2 rounded px-2 py-1.5 text-sm cursor-pointer ${on ? "bg-primary/[0.06]" : "hover:bg-muted/30"}`}>
                    <Checkbox checked={on} onCheckedChange={() => setSelecionados((prev) => { const n = new Set(prev); if (n.has(s)) n.delete(s); else n.add(s); return n; })} />
        ...
        <DialogFooter className="p-3 border-t flex-row gap-2">
          <Button type="button" variant="outline" className="flex-1" onClick={() => onOpenChange(false)} disabled={busy}>Fechar</Button>
          <Button type="button" className="flex-1 bg-brand-gradient text-white shadow-brand" onClick={carregarSelecionados} disabled={busy || selecionados.size === 0}>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={destino} onValueChange={setDestino} disabled={busy}>
              ...
              <SelectContent>
                ...
                {reports.map((r) => <SelectItem key={r.id} value={r.id}>Anexar a: {r.titulo}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          {progresso && (
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground text-center">{progresso.etapa}… {progresso.done}/{progresso.total} — não feche.</p>
            </div>
          )}
        </div>
        <DialogFooter className="flex-col-reverse sm:flex-row gap-2">
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Cancelar</Button>
          <Button type="button" onClick={compor} disabled={busy} className="bg-brand-gradient text-white shadow-brand">{busy ? "Compondo..." : "Compor RTI"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
