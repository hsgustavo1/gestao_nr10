    ...
    <PageShell>
      <Button asChild variant="ghost" size="sm" className="text-muted-foreground -ml-2">
        <Link to="/campo/inspecao/$id" params={{ id: point.inspection_id }}>
          <ArrowLeft className="h-4 w-4" /> Inspeção
        </Link>
      </Button>
      ...
      <div className="mt-2">
        ...
        <div className="mt-0.5 text-xs text-muted-foreground inline-flex items-center gap-1 flex-wrap">
          ...
          {path.map((n, i) => (
            <span key={n.id}>
              {i > 0 && " › "}
              <span title={NIVEL_LABEL[n.nivel]}>{n.nome}</span>
            </span>
          ))}
        </div>
        {point.observacoes && (
          <p className="mt-1 text-xs text-muted-foreground">{point.observacoes}</p>
        )}
      </div>
      ...
      {canEditRtiCampo && inspection?.status === "importada" && (
        <div className="mt-4 flex items-start gap-2 rounded-md border border-slate-300 bg-slate-50 p-3 text-sm text-slate-700">
          ...
          <span>
            Esta inspeção está com RTI composto. Reabra a inspeção para editar os achados e fotos.
          </span>
        </div>
      )}
        ...
        <CardContent className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold inline-flex items-center gap-1.5">
              <Camera className="h-4 w-4 text-primary" /> Fotos ({photos.length})
            </h2>
            {canEdit && (
              <>
                <input
          ...
          {photos.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              Nenhuma foto. Toque em "Foto" para anexar.
            </p>
          ) : (
                ...
                >
                  {isImage(ph) ? (
                    <button
                      ...
                      title={ph.legenda ?? ph.file_name}
                    >
                      <img
                        src={rtiFileUrl(ph.file_path)}
                        alt={ph.legenda ?? ph.file_name}
                        loading="lazy"
                    ...
                    <a
                      href={rtiFileUrl(ph.file_path)}
                      target="_blank"
                      rel="noreferrer"
                      className="flex aspect-square flex-col items-center justify-center gap-1 p-2 text-center"
                    >
                      <Camera className="h-6 w-6 text-muted-foreground" />
                      <span className="text-[9px] text-muted-foreground line-clamp-2 break-all">
                        {ph.file_name}
                      </span>
                    </a>
                  )}
      ...
      {photos.length > 0 && findings.length === 0 && (
        <div className="mt-3 flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
          ...
          <span>
            Este ponto tem {photos.length === 1 ? "1 foto" : `${photos.length} fotos`} mas nenhum
            modo de falha registrado. Adicione ao menos um modo de falha para que este ponto seja
            incluído no RTI.
          </span>
        </div>
      )}
      ...
      <div className="mt-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold inline-flex items-center gap-1.5">
          <ListChecks className="h-4 w-4 text-primary" /> Modos de falha ({findings.length})
        </h2>
        {canEdit && (
          <div className="flex gap-1">
            <Button size="sm" variant="outline" onClick={() => setModoSheetOpen(true)}>
              <Plus className="h-4 w-4" /> Da base
            </Button>
            <Button
              size="sm"
              variant="ghost"
        ...
        {findings.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-center text-sm text-muted-foreground">
              Nenhum modo de falha. Adicione da base ou manual.
            </CardContent>
          </Card>
        ) : (
          findings.map((f) => (
              ...
              <CardContent className="p-3 flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span
                      className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[11px] font-bold ${RTI_PRIORIDADE_BADGE[clampPrioridade(f.prioridade)]}`}
                    >
                      P{f.prioridade}
                    </span>
                    <span className="text-[11px] text-muted-foreground">
                      {RTI_TIPO_EXECUCAO_LABELS[f.tipo_execucao]}
                    </span>
                    {!f.modo_falha_id && (
                      <span className="inline-flex items-center rounded-full border border-slate-300 bg-slate-50 px-1.5 py-0.5 text-[10px] text-slate-600">
                        manual
                      </span>
                    )}
                  </div>
                  ...
                  {f.recomendacao && (
                    <p className="mt-1 text-[11px] text-muted-foreground leading-snug">
                      <span className="font-semibold">Recomendação:</span> {f.recomendacao}
                    </p>
                  )}
                </div>
                {canEdit && (
      ...
      {canEdit && modoSheetOpen && (
        <ModosFalhaSheet
          pointId={id}
          jaSelecionados={new Set(findings.map((f) => f.modo_falha_id).filter(Boolean) as string[])}
          onOpenChange={(o) => {
            if (!o) setModoSheetOpen(false);
          }}
        />
      )}
      {canEdit && manualOpen && (
...
function DeleteFindingButton({ finding, pointId }: { finding: FieldFinding; pointId: string }) {
  ...
  return (
    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={handle}>
      <Trash2 className="h-3.5 w-3.5" />
    </Button>
  );
}
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl w-[calc(100vw-1rem)] p-3">
        ...
        <img
          src={rtiFileUrl(photo.file_path)}
          alt={photo.legenda ?? photo.file_name}
    ...
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-1rem)] sm:max-w-lg flex flex-col p-0">
        <DialogHeader className="p-4 pb-2">
          ...
          <DialogDescription>Toque para adicionar como achado deste ponto.</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
          {isLoading ? (
            ...
            [...filtrados.entries()].map(([categoria, items]) => (
              <div key={categoria}>
                <div className="sticky top-0 bg-background py-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                  {categoria}
                </div>
                  ...
                  {items.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => adicionar(m)}
                        disabled={aplicado === m.id || ja}
                        className="w-full text-left rounded-md border p-2.5 transition-colors hover:border-primary/40 hover:bg-muted/30 disabled:opacity-50"
                      >
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto w-[calc(100vw-1rem)] sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Editar achado" : "Achado manual"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-3">
          <div className="space-y-1.5">
            <Label htmlFor="fd-desc">Não conformidade</Label>
            <Textarea
              id="fd-desc"
              value={descricao}
