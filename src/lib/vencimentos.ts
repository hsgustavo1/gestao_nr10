...
import { latestASOByEmployee, ASO_TIPO_LABELS } from "./asos";
import { TRAINING_LABELS, type ITTraining, type TrainingType, type WorkInstruction } from "./qualificacoes";
import { PIE_CATEGORY_LABELS } from "./prontuario";
import { INSPECTION_TYPE_SHORT } from "./inspecoes";
import { EPI_TYPE_LABELS, lastTestByEpi, nextTestDate } from "./epis";
 ...
 * horizonte em dias). Horizonte padrão de 90 dias.
 */
export function useVencimentos(horizonDays = 90) {
  ...
  const items = useMemo<VencimentoItem[]>(() => {
    ...
    for (const it of itTrainings as Array<ITTraining & { work_instructions?: Pick<WorkInstruction, "code" | "title"> | null }>) {
